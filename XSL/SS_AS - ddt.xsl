...
<xsl:template name="FormatDateIta">
    ...
    <!-- Correzione delle posizioni per year, month e day -->
    <xsl:variable name="year" select="substring($DateTime, 1, 4)" />
    <xsl:variable name="month" select="substring($DateTime, 6, 2)" />
    <xsl:variable name="day" select="substring($DateTime, 9, 2)" />

    <!-- Formattazione finale della data nel formato DD-MM-YYYY -->
        ...
        <tr>
          <td >
            ...
            <xsl:if test="TipoRitenuta">
              ...
              <xsl:choose>
                <xsl:when test="contains($TR, 'RT01')">
                  (ritenuta persone fisiche)
                </xsl:when>
                <xsl:when test="contains($TR, 'RT02')">
                              ...
                              <xsl:for-each select="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/DatiAnagrafici">
                                ...
                                <div class="headContent" >
                                  ...
                                  <xsl:if test="RegimeFiscale">
                                    ...
                                    <xsl:choose>
                                      <xsl:when test="contains($RF, 'RF01')">
                                        (ordinario)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF02')">
                                        (contribuenti minimi)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF03')">
                                        (nuove iniziative produttive)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF04')">
                                        (agricoltura e attività connesse e pesca)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF05')">
                                        (vendita sali e tabacchi)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF06')">
                                        (commercio fiammiferi)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF07')">
                                        (editoria)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF08')">
                                        (gestione servizi telefonia pubblica)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF09')">
                                        (rivendita documenti di trasporto pubblico e di sosta)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF10')">
                                        (intrattenimenti, giochi e altre attività di cui alla tariffa allegata al DPR 640/72)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF11')">
                                        (agenzie viaggi e turismo)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF12')">
                                        (agriturismo)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF13')">
                                        (vendite a domicilio)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF14')">
                                        (rivendita beni usati, oggetti d’arte,
                                        d’antiquariato o da collezione)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF15')">
                                        (agenzie di vendite all’asta di oggetti d’arte,
                                        antiquariato o da collezione)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF16')">
                                        (IVA per cassa P.A.)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF17')">
                                        (IVA per cassa - art. 32-bis, D.L. 83/2012)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF19')">
                                        (Regime forfettario)
                                      </xsl:when>
                                      <xsl:when test="contains($RF, 'RF18')">
                                        (altro)
                                      </xsl:when>
                                      <xsl:when test="$RF=''">
                                      </xsl:when>
              ...
              <tr>
                <td>
                  <xsl:if test="DatiGenerali/DatiGeneraliDocumento/TipoDocumento">
                    ...
                    <xsl:variable name="TD" select="DatiGenerali/DatiGeneraliDocumento/TipoDocumento" />
                    <xsl:choose>
                      <xsl:when test="contains($TD, 'TD01')">
                        (fattura)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD02')">
                        (acconto/anticipo su fattura)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD03')">
                        (acconto/anticipo su parcella)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD04')">
                        (nota di credito)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD05')">
                        (nota di debito)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD06')">
                        (parcella)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD20')">
                        (autofattura)
                      </xsl:when>
                      <!--FPRS-->
                      <xsl:when test="contains($TD, 'TD07')">
                        (fattura semplificata)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD08')">
                        (nota di credito semplificata)
                      </xsl:when>
                      <xsl:when test="contains($TD, 'TD09')">
...
<!-- Codice per generare la tabella Fornitore e Protocollo -->
<xsl:if test="//Fornitore and //Protocollo">
    ...
    <fo:table-body>
      <fo:table-row>
        <fo:table-cell padding="5pt">
          <!-- Fornitore -->
          <fo:block font-size="10pt" font-weight="bold" text-align="center">F:</fo:block>
        </fo:table-cell>
        <fo:table-cell padding="5pt">
          <fo:block font-size="20pt" text-align="center">
            <xsl:value-of select="//Fornitore"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
      <fo:table-row>
        <fo:table-cell padding="5pt">
          <!-- Protocollo -->
          <fo:block font-size="10pt" font-weight="bold" text-align="center">| P:</fo:block>
        </fo:table-cell>
        <fo:table-cell padding="5pt">
          <fo:block font-size="20pt" text-align="center">
            <xsl:value-of select="//Protocollo"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
    </fo:table-body>
...
<!-- Aggiungi uno spazio tra la sezione Fornitore/Protocollo e DDT -->
<fo:block margin-top="10pt"/>

<!-- Codice XSLT per generare la tabella DDT solo se esiste almeno un campo NumeroDDT -->
<xsl:if test="//NumeroDDT">
  <fo:table table-layout="fixed" width="100%" border-collapse="separate" border-spacing="2pt">
    <fo:table-column column-width="100%"/>
    <fo:table-header>
      <fo:table-row>
        <fo:table-cell padding="5pt" border="1pt solid black" background-color="#f0f0f0">
          <fo:block font-weight="bold" font-size="10pt" text-align="center">| DDT: </fo:block>
        </fo:table-cell>
      </fo:table-row>
    </fo:table-header>
    <fo:table-body>
      <fo:table-row>
        <fo:table-cell padding="5pt" border="1pt solid black">
          <fo:block text-align="center" font-size="10pt">
            <!-- Logica per concatenare tutti i campi NumeroDDT unici separati da virgola -->
            <xsl:for-each select="//NumeroDDT">
              <xsl:if test="not(. = preceding::NumeroDDT)">
                <xsl:value-of select="." />
...
<!-- Aggiungi uno spazio dopo la tabella DDT -->
                        ...
                        <td colspan="3" >
                          <xsl:choose>
                            <xsl:when test="EsigibilitaIVA">
                              ...
                              <xsl:choose>
                                <xsl:when test="contains($EI, 'I')">
                                  (esigibilità immediata)
                                </xsl:when>
                                <xsl:when test="contains($EI, 'D')">
                                  (esigibilità differita)
                                </xsl:when>
                                <xsl:when test="contains($EI, 'S')">
